...
public class FirstLabFragment extends Fragment implements UsersAdapter.OnUserSelectedListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView =
                inflater.inflate(R.layout.fragment_first_lab, container, false);


        MaterialSpinner spinnerGroup = (MaterialSpinner) getActivity().findViewById(R.id.spinnerGroup);
        mUsersRecycle = rootView.findViewById(R.id.recycler_first_lab);
        mEmptyView = rootView.findViewById(R.id.viewEmptyUsers1);
        mFirestore = FirebaseFirestore.getInstance();
        mQuery = mFirestore.collection("users").whereEqualTo("group",spinnerGroup.getText().toString());

        mAdapter = new UsersAdapter(mQuery, this, 1) {
            @Override
